"""DiffusionGemma SFT config — visual Sudoku (add-visual-inputs design).

The ``sft_sudoku_vision`` target config from
docs/add-visual-inputs/index.html: same task as
``gemma/diffusion/.../configs/sft_sudoku_full.py``, but the puzzle arrives
as ONE image of the 9x9 grid — every diff against the baseline config is a
vision-input change and nothing else:

  * ``text_only=False`` on the backbone — keeps the SigLIP-style vision
    tower (+572.8M params, fp32, trainable) and mm projection in the param
    tree.
  * ``aux.prompt_len`` 256 -> 384 — the prompt must also hold the expanded
    image span (S_v + 4 <= 284) plus the instruction text.
  * ``aux.max_soft_tokens_per_image = 280`` — preprocessing budget; must
    equal ``vision_encoder.output_length``.
  * batch fields ``patches [B, P_p, p_d]`` / ``positions_xy [B, P_p, 2]``,
    bound via the ``batch.patches`` / ``batch.positions_xy`` kontext keys.
  * the vision-sudoku data pipeline (``make_sudoku_vision_ds``).
  * the vision variants of the SFT model / network / evaluators.

Denoiser passes, self-conditioning, corruption, DDIM stepper, both loss
...
with konfig.imports():
  from ft_gemma.diffusion.hackable_diffusion_adapter.data.sudoku import sudoku_vision_data
...
def get_config():
  """SFT config for visual Sudoku solving."""
  ...
  # MODIFIED (design §8): 256 -> 384 — must also hold the expanded image
  # span (S_v + 4 <= 284) plus the instruction text.
  cfg.aux.prompt_len = 384
  cfg.aux.num_canvases = 1
  cfg.aux.canvas_size = 256
  # NEW (design §8): preprocessing budget — max_patches = 280 * 3**2 = 2520;
  # must equal vision_encoder.output_length.
  cfg.aux.max_soft_tokens_per_image = 280
  use_lora = False
  lora_rank = 8
  ...
  # MODIFIED: the prompt template carries a literal <|image|> placeholder
  # instead of the puzzle digits — the {text} slot receives "<|image|>".
  ...
  # NEW: text_only=False keeps config.vision_encoder + mm projection in the
  # param tree; the wrapper threads images= into Transformer.__call__.
  base_network = vision_hd_gemma_network.VisionWrappedDiffusionGemmaNetwork(
      gemma_model=vision_gemma_model.VisionDiffusionGemma_26B_A4B(
          text_only=False,
      ),
  )
  if use_lora:
  ...
  cfg.model = vision_sft_model.VisionSFTDiffusion(
      x0="batch.canvas",
      prompt="batch.prompt",
      ...
      # NEW: bind the image tensors into SFTDiffusion.__call__ -> sft_encode.
  ...
  # MODIFIED: DIFFUSIONGEMMA_26B_A4B_IT is expected to also cover
  # vision_encoder/* + embedder/mm_input_projection (design §0; the loader
  # raises on any model param missing from the checkpoint).
  cfg.init_transform = gemma_checkpointer.GemmaDiffusionCheckpointLoader(
      path=CHECKPOINT_PATH,
  )

  cfg.train_ds = sudoku_vision_data.make_sudoku_vision_ds(
      bagz_path="ft_gemma/diffusion/hackable_diffusion_adapter/data/sudoku/sudoku_vision_train.bagz",
      training=True,
      batch_size=8,
      prompt_len=cfg.ref.aux.prompt_len,
      num_canvases=cfg.ref.aux.num_canvases,
      ...
      max_soft_tokens_per_image=cfg.ref.aux.max_soft_tokens_per_image,
  )
  cfg.eval_ds = sudoku_vision_data.make_sudoku_vision_ds(
      bagz_path="ft_gemma/diffusion/hackable_diffusion_adapter/data/sudoku/sudoku_vision_eval.bagz",
      training=False,
      batch_size=8,
      prompt_len=cfg.ref.aux.prompt_len,
      num_canvases=cfg.ref.aux.num_canvases,
  ...
  all_evals = vision_ar_eval.make_ar_evals(
      cfg,
  ...
  early_stopping_evals = vision_ar_eval.make_ar_evals(
      cfg,
      gemma_network_ref=cfg.ref.model.gemma_network,
      corruption_process_ref=cfg.ref.aux.corruption_process,
      canvas_size_ref=cfg.ref.aux.canvas_size,
