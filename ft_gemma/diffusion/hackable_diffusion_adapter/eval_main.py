r"""Evaluation binary for the vision-input Hackable Diffusion adapter.

Mirror of ``gemma/diffusion/hackable_diffusion_adapter/eval_main.py`` that
injects the *vision* AR evaluators (``vision_ar_eval.make_ar_evals``) instead
of the text ones: the vision evaluator adds the patch tensors to the
conditioning dict, so the prompt prefill inside ``init_ar_state`` embeds and
merges the image before the first canvas (design §6, §11).

Runs AR sampling evaluation on a single checkpoint and reports task-specific
metrics (accuracy, etc.) plus text sample summaries.
"""
...
def _sudoku_metrics():
  """Return task-specific metrics for the visual-Sudoku config."""
  return {
...
def _inject_ar_evals(cfg, task):
  """Build vision AR evaluators with task metrics and add them to ``cfg``.

  Args:
    cfg: The (unresolved) Kauldron trainer config.
    task: Task name (``"sudoku"``).
  """
